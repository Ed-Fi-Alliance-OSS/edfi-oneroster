...
- `-EnvFile` lets you point at any dotenv file (defaults to `.env` inside this
  folder). Relative paths are resolved from `compose/`.
...
| `nginx-compose.yml` | HTTPS reverse proxy that fronts all APIs and terminates TLS using the templates under `compose/ssl`. | `nginx`
...
- `compose/.env.5.2.0.example` – Defaults for Ed-Fi Data Standard 5.2.0.
- `compose/.env.4.0.0.example` – Defaults for Data Standard 4.0.0.
...
| **Logging & TLS trust** | `LOGS_FOLDER`, `NODE_EXTRA_CA_CERTS`, `TRUST_PROXY` | `LOGS_FOLDER` is bind-mounted into `v7-single-api`; `NODE_EXTRA_CA_CERTS` points at the self-signed CA bundled under `compose/ssl`. |
...
2. Ensure the required `.crt`, `.key`, and `.pem` files exist under
   `compose/ssl`; run `./generate-certificate.sh` in that folder to create fresh
