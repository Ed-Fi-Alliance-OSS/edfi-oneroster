...
- **`roles` are deduped per org.** The `roles` array carries one entry per distinct associated org — the `*_orgs`/`contact_orgs` sources are deduped to one row per `(person, school)` (staff additionally by `classification`) before aggregation, so a re-enrollment (or a contact linked to two students at the same school) does not emit duplicate org entries. On both engines the array is built set-based (PostgreSQL `json_agg … GROUP BY`; **MSSQL** `STRING_AGG` over the deduped orgs with the primary school precomputed), replacing an earlier per-row correlated `FOR JSON` subquery on MSSQL (ONEROSTER-107).
- **MSSQL refresh is fully materialized (performance, ONEROSTER-107).** `sp_refresh_users` builds each intermediate into indexed `#temp` tables — the `student_*`/`staff_*`/`contact_*`(`parent_*`) sets, the staff-classification chain, and the per-type roles aggregates (`#student_orgs_agg`, `#staff_orgs_agg`, `#parent_roles`) — then runs three independent `INSERT`s (student/staff/parent) into a staging table. The old single `WITH … UNION ALL … INSERT` didn't scale on large ODSs: because **SQL Server does not materialize CTEs**, the staff-classification chain was re-expanded as a correlated subquery per staff row, producing a ~10¹²-row cardinality mis-estimate and a multi-terabyte sort grant that spilled tempdb; materializing each step gives the optimizer real row counts. The staging table loads as a **heap** with a `UNIQUE NONCLUSTERED` index added _after_ load, since a clustered PK on the random `md5` `sourcedId` sorts and page-splits the wide (`NVARCHAR(MAX)`) rows on every insert — a cost that grows with row count. PostgreSQL's materialized view already computes these once (multiply-referenced CTEs are materialized), so it needs no equivalent change.
