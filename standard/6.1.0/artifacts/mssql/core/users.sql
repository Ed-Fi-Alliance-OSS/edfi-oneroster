...
-- Authorization filters: education organization lookups
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- DS 6.1: StudentEducationOrganizationAssociationStudentIdentificationCode was
        -- promoted to edfi.StudentIdentificationCode. Its PK narrowed to
        -- (EducationOrganizationId, StudentIdentificationSystemDescriptorId, StudentUSI) --
        -- AssigningOrganizationIdentificationCode is no longer part of the key -- so a
        -- student now contributes at most one entry per identification system per org.
        SELECT
            sid.StudentUSI,
            sid.EducationOrganizationId,
            (SELECT
                d2.CodeValue AS type,
                sid2.IdentificationCode AS identifier
             FROM edfi.StudentIdentificationCode sid2
                JOIN edfi.Descriptor d2 ON sid2.StudentIdentificationSystemDescriptorId = d2.DescriptorId
             WHERE sid2.StudentUSI = sid.StudentUSI
               AND sid2.EducationOrganizationId = sid.EducationOrganizationId
             ORDER BY d2.CodeValue
             FOR JSON PATH) AS ids
        INTO #student_ids
        FROM edfi.StudentIdentificationCode sid
        GROUP BY sid.StudentUSI, sid.EducationOrganizationId;
        ...
        -- Preferred staff email (Work first), resolved per (staff, education organization).
        -- DS 6.1: StaffElectronicMail and StaffEducationOrganizationContactAssociation were
        -- both replaced by edfi.StaffDirectoryElectronicMail, which adds
        -- EducationOrganizationId to the key so contact details can vary by organization.
        -- Staff user rows are emitted per (staff, school), so the preference is resolved
        -- per organization and joined on the school below -- picking one address per person
        -- would stamp one school's address onto every school row for that staff member.
        WITH ranked_staff_email AS (
            SELECT
                seo.StaffUSI,
                seo.EducationOrganizationId,
                seo.ElectronicMailAddress,
                ROW_NUMBER() OVER (
                    PARTITION BY seo.StaffUSI, seo.EducationOrganizationId
                    ORDER BY
            ...
            FROM edfi.StaffDirectoryElectronicMail seo
                JOIN edfi.Descriptor d
                    ON seo.ElectronicMailTypeDescriptorId = d.DescriptorId
            WHERE seo.ElectronicMailAddress IS NOT NULL
        )
        ...
        INTO #staff_email
        FROM ranked_staff_email
        WHERE org_rank = 1
        UNION ALL
        -- Person-level fallback row, keyed with a NULL organization. Staff with no
        -- StaffSchoolAssociation produce a person-keyed user row (sso.SchoolId IS NULL),
        -- which has no organization to scope to; without this row they would lose the
        -- address they had under 5.2. School-keyed rows never match it.
        SELECT StaffUSI, CAST(NULL AS BIGINT), ElectronicMailAddress
        FROM ranked_staff_email
        WHERE person_rank = 1;
        CREATE CLUSTERED INDEX IX_tmp_staff_email ON #staff_email (StaffUSI, EducationOrganizationId);
        ...
        -- Staff identification codes (additional identifiers like State ID), resolved per
        -- (staff, education organization).
        -- DS 6.1: StaffIdentificationCode gained EducationOrganizationId in its PK, so the
        -- same identifier can be recorded at several organizations. Aggregating by StaffUSI
        -- alone would repeat it once per organization in the userIds array. This mirrors
        -- #student_ids above, which has always been organization-scoped.
        --
        -- The driver also emits one NULL-organization row per staff member. Staff with no
        -- StaffSchoolAssociation produce a person-keyed user row (sso.SchoolId IS NULL) that
        -- has no organization to scope to; without that row they would lose the identifiers
        -- they had under 5.2. School-keyed rows never match it. The NULL row is deduped with
        -- DISTINCT so an identifier recorded at several organizations still appears once.
        SELECT
            drv.StaffUSI,
            drv.EducationOrganizationId,
            (
                SELECT
                    JSON_QUERY(
                        (SELECT DISTINCT
                         ...
                         WHERE sic.StaffUSI = drv.StaffUSI
                           AND (drv.EducationOrganizationId IS NULL
                                OR sic.EducationOrganizationId = drv.EducationOrganizationId)
        ...
        INTO #staff_ids
        FROM (
            SELECT DISTINCT StaffUSI, EducationOrganizationId FROM edfi.StaffIdentificationCode
            UNION ALL
            SELECT DISTINCT StaffUSI, CAST(NULL AS BIGINT) FROM edfi.StaffIdentificationCode
        ) drv;
        CREATE CLUSTERED INDEX IX_tmp_staff_ids ON #staff_ids (StaffUSI, EducationOrganizationId);
        ...
        -- Preferred contact email (primary, publishable)
        ...
        WHERE x.email_rank = 1;
        CREATE CLUSTERED INDEX IX_tmp_contact_email ON #contact_email (ContactUSI);

        -- Contact org associations, deduped to one row per (contact, school) first, so a
        -- contact linked to a student with multiple enrollments (or to two students at
        ...
        CREATE CLUSTERED INDEX IX_tmp_contact_orgs ON #contact_orgs (ContactUSI);

        -- Contact primary org (keys the single contact user row)
        SELECT ContactUSI, SchoolId
        INTO #contact_primary_org
        FROM #contact_orgs
        WHERE seq = 1;
        CREATE CLUSTERED INDEX IX_tmp_contact_primary_org ON #contact_primary_org (ContactUSI);

        -- roles JSON per contact
        SELECT
            co.ContactUSI,
            '[' + STRING_AGG(
                CAST(
        ...
        CREATE CLUSTERED INDEX IX_tmp_parent_roles ON #parent_roles (ContactUSI);
        ...
        FROM edfi.staff st
            -- #staff_school must be joined before #staff_email and #staff_ids: both are
        ...
        FROM edfi.contact c
            LEFT JOIN #contact_email ce ON c.contactusi = ce.contactusi
            LEFT JOIN #parent_roles pr ON c.ContactUSI = pr.ContactUSI
            LEFT JOIN #contact_primary_org cpo ON c.ContactUSI = cpo.ContactUSI
        ;
