...
-- =============================================
    ...
    ALTER TABLE oneroster12.demographics ADD CONSTRAINT PK_demographics PRIMARY KEY (sourcedId);
    PRINT '  ✓ Created primary key on demographics';
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- across ALL of the student's demographic records regardless of the org level
        -- (school, LEA, ESC, SEA) they were recorded at, so higher-org data is never
        -- dropped.
        -- DS 6.1: HispanicLatinoEthnicity moved off StudentEducationOrganizationAssociation
        -- onto edfi.StudentDemographic, which is keyed the same way on
        -- (EducationOrganizationId, StudentUSI), so the roll-up below is unchanged.
        student_hispanic AS (
            SELECT
                sd.StudentUSI,
                CAST(MAX(CAST(sd.HispanicLatinoEthnicity AS INT)) AS BIT) AS hispaniclatinoethnicity,
                MAX(sd.LastModifiedDate) AS edorg_lmdate
            FROM edfi.StudentDemographic sd
            GROUP BY sd.StudentUSI
        ),
        -- Race is likewise person-level: union the mapped race descriptors across all
        -- of the student's demographic records.
        student_race AS (
            SELECT
                seoar.StudentUSI,
            ...
            FROM edfi.StudentDemographicRace seoar
