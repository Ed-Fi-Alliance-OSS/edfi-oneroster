...
$dsFolder = if ($DataStandard -eq 'ds4') { 'ds4' } else { 'ds5' }
$dsDir = Join-Path $scriptDir $dsFolder

$sqlFiles = @()
if (Test-Path $coreDir) {
  $sqlFiles += Get-ChildItem -Path $coreDir -Filter *.sql | Sort-Object Name | ForEach-Object { $_.FullName }
}
if (Test-Path $dsDir) {
  $sqlFiles += Get-ChildItem -Path $dsDir -Filter *.sql | Sort-Object Name | ForEach-Object { $_.FullName }
}
  ...
  foreach ($file in $sqlFiles) {
    ...
    if (Test-Path -LiteralPath $file) {
      ...
      if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "❌ Error executing $fileName"
        Write-Host "SQL Error Output:"
        ($execOutput -split "`n" | ForEach-Object { "  $_" }) | Write-Host
        $failed++
      }
    } else {
      Write-Host "⚠️  Skipping $fileName (file not found)"
      $failed++
    }
  }
}
